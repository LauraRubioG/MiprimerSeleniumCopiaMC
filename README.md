...
* **`clickLogin()`** 🖱️: Ejecuta la acción de pulsado del botón.
...
#### 1. ¿Qué hace la anotación `@BeforeEach`? ⏱️
...
| :--- | :--- | :--- |
| **`findElement()`** | El **primer** `WebElement` encontrado. | Lanza una excepción `NoSuchElementException` 🚫 |
| **`findElements()`** | Una **lista** (`List<WebElement>`). | Devuelve una lista vacía (permite continuar). 📄 |

#### 4. ¿Por qué utilizamos una clase `LoginPage` en lugar de escribir todo en el test? 🏗️
...
**1. Reutilización de código (DRY - Don't Repeat Yourself)** ♻️
...
* **Con LoginPage:** Defines el selector y el método `login()` **una sola vez**. Si algo cambia en la interfaz, solo lo correges en un lugar y todos los tests se actualizan automáticamente.
